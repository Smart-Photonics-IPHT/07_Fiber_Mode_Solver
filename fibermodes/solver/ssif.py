...
class Neff(FiberSolver):
    ...
    def _HEpoint(self, wl, nu, neff, r):
        ...
        if r > rho:
            pz = ((a1**2) / 2 * Y0) * ((nco2 * u2)/(neff * knw2 * w2)) * ((a1 * a5 * kmur2) + (a2 * a6 * kpur2) + (((1 - 2 * Delta - F1 * F2)/2) * kmur * kpur))
        else:
            pz = ((a1**2) / 2 * Y0) * (nco2/(neff * jnu2)) * (a1 * a3 * jmur2) + ((a2 * a4 * jpur2) + (((1 - F1 * F2)/2) * jmur * jpur))
    ...
    def _TEpoint(self, wl, nu, neff, r):
        ...
        if r > rho:
             pz = ((a1**2) / 2 * Y0) * (k1wr2 * neff / k1w2)
        else:
            pz = ((a1**2) / 2 * Y0) * (j1ur2 * neff / j1u2)
    ...
    def _TMpoint(self, wl, nu, neff, r):
        ...
        if r > rho:
            pz = ((a1**2) / 2 * Y0) * (nco2 / neff - neff * 2 * Delta) * (k1wr2 / k1w2)
        else:
            pz = ((a1**2) / 2 * Y0) * (nco2 / neff) * (j1ur2 / j1u2)
        return numpy.array((0, 0, pz))#, numpy.array((0, 0, 0))
