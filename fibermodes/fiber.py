...
class Fiber(object):
    ...
    def RelDelta_n(self, mode, wl):
        neff = self.neff(mode, wl)
        nclad = self.minIndex(-1, wl)
        nco = max(layer.maxIndex(wl) for layer in self.layers)
        return (neff - nclad)/(nco - nclad)
    ...
    def findModes(self, families, wl, numax=None, mmax=None, delta=1e-6, N_treshold=0.20):
        ...
        for fam in families:
            for nu in count(0):
                ...
                for m in count(1):
                    ...
                    mode = Mode(fam, nu, m)
                    try:
                        RelDelta_n = self.RelDelta_n(mode, wl)
                        veff = self.Veff(mode, wl)
                        co = self.cutoff(mode)
                        if co > v0 or RelDelta_n < N_treshold:  #veff < 1: was Updated by Smart_photoincs
                            break
